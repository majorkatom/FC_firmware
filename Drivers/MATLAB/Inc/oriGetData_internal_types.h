...
typedef struct {
  ...
  float AccelerometerNoise;
  float GyroscopeNoise;
  float GyroscopeDriftNoise;
  float LinearAccelerationNoise;
  float LinearAccelerationDecayFactor;
  float pQw[144];
  float pQv[36];
  ...
  float MagnetometerNoise;
  float MagneticDisturbanceNoise;
  float MagneticDisturbanceDecayFactor;
  float ExpectedMagneticFieldStrength;
  float pMagVec[3];
} ahrsfilter;
